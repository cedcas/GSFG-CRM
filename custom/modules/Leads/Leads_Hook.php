...
/*
 * 4/13/2014 - CPC
 * Modified the after meeting from +15 to +10min;
 */
...
class Leads_Hook
{
    ...
    function set_seminar_title(&$bean, $event, $arguments) {
         ...
         if (!empty($bean->gsf_semina6647details_ida)) {
            ...
            $minus = strtotime('+10 minute', $after_stamp);
    ...
    /*
     * 2/3/2014 - CPC
     * this is a custom hook that sets the number of days since the initial contact
     *
     * 3/14/2014 - CPC
     * added an if statement in order to single out the population
     * of days only when status='Follow Up';
     */
    function setDaysSinceInitialContact(&$bean, $event, $arguments) {
        $status = $bean->status;
        
     	if($status == 'Follow up'){
		$initial_contact = $bean->initial_contact;
		$days_since_entered = round((strtotime(date("Y-m-d"))-strtotime(date("Y-m-d", strtotime($initial_contact)))) / (60*60*24),0);
		$bean->lead_days_since_initial_contact = $days_since_entered;
	}else{
		$bean->lead_days_since_initial_contact = '';
		//$bean->initial_contact = '';
	}
        $bean->save();
