...
class Accounts_Hook
{
    ...
    /*
     * 10/14/2013 - CPC
     * NULL Anniversary Day when not a Client anymore to exclude from PDF/Task creation;
     */
    function setAnniversaryDayToNull(&$bean, $event, $arguments) {
       if($bean->status<>'Client') {
            $SQL_update_account = "
                UPDATE accounts SET account_days_left_to_anniv = NULL WHERE id = '$bean->id'
            ";
            $bean->db->query($SQL_update_account, true);
       }
