...
function template_echo_slice_activities_shared(& $args) {
	...
	foreach ($args['slice']->acts_arr[$shared_resource->id] as $act) {
		// Pradeep: time start bug fix start
		
		$date_start = $act->sugar_bean->date_start;
		$date_unix = to_timestamp_from_uf($date_start);

		$act->sugar_bean->time_start = timestamp_to_user_formated2($date_unix,$GLOBALS['timedate']->get_time_format());
		
		
		// Pradeep: time start bug fix end
		...
		if($act->sugar_bean->object_name == 'Call') { 
			...
			if(empty($act->sugar_bean->name)) {
				...
				echo $act->sugar_bean->time_start; 
				echo "</td></tr>";
			} else {
				echo "<td class=\"monthCalBodyDayItemTd\" width=\"100%\">
