	 ...
	 function display() {
		...
		$this->ss->assign('jsonlanguage', json_encode_kinamu($returnArray));	 		
 		...
 		foreach($arrayList as $thisList){
 			...
 			if($this->bean->fieldNameMap[$thisList['fieldid']]['fields_name_map_entry']['type'] == 'currency' || $this->bean->fieldNameMap[$thisList['fieldid']]['fields_name_map_entry']['kreporttype'] == 'currency')
		...
		if(isset($mapDetails['hasMap']) && $mapDetails['hasMap'] == true )
		{
 		...
 		if(isset($this->bean->chart_layout) && $this->bean->chart_layout != '-' && $this->bean->chart_layout != '')
 		{
 			...
 			$thisChartArray = new KReportChartArray($this->bean, $this->json_decode_kinamu(html_entity_decode_utf8($this->bean->chart_params_new)), $this->bean->chart_height, $this->bean->chart_layout);
 		...
 		$this->ss->assign('viewJS', $this->setFormatVars() . '<script type="text/javascript" src="modules/KReports/views/view.detail.' .  $this->bean->listtype . '.js"></script>');
