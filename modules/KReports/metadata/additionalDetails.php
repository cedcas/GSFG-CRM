...
function additionalDetailsKMeetingNotes($fields) {
	static $mod_strings;
	global $app_list_strings;
	if(empty($mod_strings)) {
		global $current_language;
		$mod_strings = return_module_language($current_language, 'KMeetingNotes');
	}
...
//    if(!empty($fields['DATE_START'])) $overlib_string .= '<b>'. $mod_strings['LBL_START_DATE_AND_TIME'] . '</b> ' . $fields['DATE_START'] .  '<br>';
	...
	if(!empty($fields['DESCRIPTION'])) { 
		...
		$editLink = "index.php?action=EditView&module=KMeetingNotes&record={$fields['ID']}"; 
	$viewLink = "index.php?action=DetailView&module=KMeetingNotes&record={$fields['ID']}";	

	$return_module = empty($_REQUEST['module']) ? 'KReminders' : $_REQUEST['module'];
