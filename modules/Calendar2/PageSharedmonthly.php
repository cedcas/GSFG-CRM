...
global $currentModule, $current_language, $app_strings;
	...
	while($curr_time_g < $month_end_unix){
		...
		echo "<div class='left_time_col'>";
			if ($current_language == "ja") {
				echo "<div class='day_head'><a href='index.php?module=Calendar2&action=index&view=week&hour=0&day=".timestamp_to_user_formated2($curr_time_g,'j')."&month=".timestamp_to_user_formated2($curr_time_g,'n')."&year=".timestamp_to_user_formated2($curr_time_g,'Y')."'>".timestamp_to_user_formated2($curr_time_g,'W').$app_strings['LBL_WEEK']."</a></div>";
			} else {
			...
			echo "<div class='day_col'>";
				...
				for($i = $hour_start; $i <= $hour_end; $i++){
					if($i == $hour_end && $minute_end == 0)
						break;
					if(!($i == $hour_start && $minute_start >= 0 + $t_step)){
						echo "<div id='t_".$curr_time.$un_str."' class='".$un_class."' lang='".timestamp_to_user_formated2($curr_time,$GLOBALS['timedate']->get_time_format($meridian))."' datetime='".timestamp_to_user_formated2($curr_time)."'></div>";
					}
					$curr_time += $t_step*60;
					for($j = $t_step; $j < 60; $j += $t_step){
						if($i == $hour_end && $j >= $minute_end)
