...
function fill_contact_ce(){
	$("#sel_contact_list option").remove();
	var users = find_contacts();
		$.each(
			users,
			function(j,v){
				if(!($("#sel_contact_list option[value='" + v.id + "']").length))
					$("<option>")
					.val(v.id)
					.html(v.name)
					.appendTo("#sel_contact_list");					
			}
		);
}
...
function add_contact(){
	var select_id = 'sel_contact_list';	
	var type="Contacts"
	$.each(
		$("#" + select_id + " option:selected"),
		function(i,v){
					if(!($("#sel_contact_list_selected option[value='" + $(v).val() + "']").length))
						$("<option>")
						.val($(v).val())
						.attr("type",type)
						.html($(v).html())
						.appendTo("#sel_contact_list_selected");						
		}	
	);
...
/*	if($("#sel_contact_list_selected option:selected").length == $("#sel_contact_list_selected option").length){
		alert(lbl_remove_participants);
		return;
	}
*/
	$("#sel_contact_list_selected option:selected").remove();
...
function hide_sch_div()
{
	$("#scheduler").hide();
	$(".schedulerInvitees").hide();
	$("#list_div_win").hide();
}
function show_sch_div(val)
{
	$("#scheduler").show();
	if(val==5)
	{
		$(".schedulerInvitees").show();
		$("#list_div_win").show();
	}	
	else	
	{
...
function handle_btn_remove(){
	...
	if($("#sel_contact_list_selected option").length > 0)
		$("#btn_remove_contact").removeAttr("disabled");
	else
		$("#btn_remove_contact").attr("disabled","disabled");
...
SugarWidgetSchedulerSearch.submit=function(form)
{
	...
	var query={"modules":["Contacts","Leads"],"group":"and","field_list":['id','full_name','email1','phone_work'],"conditions":conditions};
