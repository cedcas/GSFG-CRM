...
// caldav settings values
$caldav_sync_opt = $current_user->getPreference('caldav_sync_opt');
if(!isset($caldav_sync_opt) || $caldav_sync_opt =="")
	$caldav_sync_opt = 1;

$caldav_prioriy = $current_user->getPreference('caldav_prioriy');	
if(!isset($caldav_prioriy) || $caldav_prioriy=="")
	$caldav_prioriy = 2;

$caldav_sync_mod = $current_user->getPreference('caldav_sync_mod');
if(!isset($caldav_sync_mod))
	$caldav_sync_mod = "call";
	
$caldav_time_slot = $current_user->getPreference('caldav_time_slot');
if(!isset($caldav_time_slot) || $caldav_time_slot=="")
	$caldav_time_slot=2;	

$caldav_url = $current_user->getPreference('caldav_url');
$caldav_username = $current_user->getPreference('caldav_username');
$caldav_password = $current_user->getPreference('caldav_password');
...
if($_REQUEST['view'] == "week" ||  $_REQUEST['view'] == "day" || $_REQUEST['view'] == "month" || $_REQUEST['view'] == "year" || $_REQUEST['view'] == "shared" || $_REQUEST['view'] == "sharedmonthly"){
	...
	?>
	&nbsp;&nbsp;&nbsp;&nbsp;<input class="button" type="button" value=" <?php echo $mod_strings['LNK_VIEW_CALENDAR'] ?> " title="<?php echo $mod_strings['LBL_TODAY']; ?>" onclick="window.location.href='index.php?module=<?php echo $currentModule ?>&action=index&view=<?php echo $args['calendar']->view;?>&day=<?php echo intval($today_arr['day']);?>&month=<?php echo intval($today_arr['month']);?>&year=<?php echo $today_arr['year'];?>'">&nbsp;
	<?php
	echo "</div>";
	echo "<div style='float:left; text-align: right; width: 50%; font-size: 12px;'>";	
	if($args['view'] != 'shared' && $args['view'] != 'sharedmonthly' && $args['view'] != 'year' ){
	?>
		<input class="button" type="button" id="toggle_search" value=" <?php echo $mod_strings['LBL_SHOW_SEARCH'];?> " title="<?php echo $GLOBALS['app_strings']['LBL_SEARCH'];?>" onclick="switchSearch();">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	<?php
	}
	echo "<a href='#' onClick='$(\"#settings_dialog\").dialog(\"open\");'>".$mod_strings['LBL_SETTINGS']."</a>";
	echo "&nbsp;&nbsp;&nbsp;&nbsp;</div>";
	...
	<form action='index.php' method='POST' name='contact_search' id='contact_search' class='search_form' >
	<div class='edit view search' style='border-top: 2px solid;'>
	<table cellpadding=0 celspacing=0 border=0>
";
	echo "<tr>";
	if ($current_language == "ja") {
		echo "<td class='dataLabel' nowrap='nowrap' width='10%'> ".$con_lang['LBL_LIST_LAST_NAME']." </td>";
		echo "<td class='dataField' nowrap='nowrap' width='30%'><input type='text' size='30' id='last_name_search' name='last_name_search'></td>";
		echo "<td class='dataLabel' nowrap='nowrap' width='10%'> ".$con_lang['LBL_LIST_FIRST_NAME']." </td>";
		echo "<td class='dataField' nowrap='nowrap' width='30%'><input type='text' size='30' id='first_name_search' name='first_name_search'></td>";
	} else {
		echo "<td class='dataLabel' nowrap='nowrap' width='10%'> ".$con_lang['LBL_LIST_FIRST_NAME']." </td>";
		echo "<td class='dataField' nowrap='nowrap' width='30%'><input type='text' size='30' id='first_name_search' name='first_name_search'></td>";
		echo "<td class='dataLabel' nowrap='nowrap' width='10%'> ".$con_lang['LBL_LIST_LAST_NAME']." </td>";
	...
	echo "<td class='dataLabel' nowrap='nowrap' width='10%'> ".$app_strings['LBL_CURRENT_USER_FILTER']." </td>";
	echo "<td class='dataField' nowrap='nowrap' width='30%'><input type='checkbox' id='current_user_only_search' name='current_user_only_search' ></td>";
	...
	echo "<tr>";
	echo "<td class='dataLabel' nowrap='nowrap'> ".$con_lang['LBL_LIST_ACCOUNT_NAME']." </td>";
	//echo "<td><input type='text' size='30' id='account_name_search' name='account_name_search'></td>";
	?>
	<td class='dataField' nowrap='nowrap'><input type="text" name="account_name_search" class="sqsEnabled" id="account_name_search" size="" value="" title='' autocomplete="off"  >
	<input type="hidden" name="account_id_search" id="account_id_search" value="">
	<input type="button" name="btn_account_name" id="btn_account_name" tabindex="102" title="<?php echo $GLOBALS['app_strings']['LBL_SELECT_BUTTON_TITLE']; ?>" accessKey="T" class="button" value="<?php echo $GLOBALS['app_strings']['LBL_SELECT_BUTTON_LABEL']; ?>" onclick='open_popup("Accounts", 600, 400, "", true, false, {"call_back_function":"set_return","form_name":"contact_search","field_to_name_array":{"id":"account_id_search","name":"account_name_search"}}, "single", true);' >
	<input type="button" name="btn_clr_account_name" id="btn_clr_account_name" tabindex="102" title="<?php echo $GLOBALS['app_strings']['LBL_CLEAR_BUTTON_TITLE']; ?>" accessKey="C" class="button" onclick="this.form.account_name_search.value = ''; this.form.account_id_search.value = '';" value="<?php echo $GLOBALS['app_strings']['LBL_CLEAR_BUTTON_LABEL']; ?>" ></td>
	...
	<?php
echo "
	<button class='button' type='button' onclick='searchContacts(0);'>".$GLOBALS['app_strings']['LBL_SEARCH']."</button>
	<button class='button' type='button' onclick='clearSearchFields(0); clearContacts(0);'>".$GLOBALS['app_strings']['LBL_CLEAR_BUTTON_LABEL']."</button>
";
	?>
	<table style='width: 100%' cellspacing="0" cellpadding="0" border="0" class="list view contactsTable">
		<tr class='pagination'>
			<td></td>
			<td></td>
			<td></td>
			<td align='right' colspan=2>	
				<div style='padding: 2px;'>	
						<button id='scFirst' type="button" title="<?php echo $app_strings['LNK_LIST_START']; ?>" class="button" disabled="disabled">
							<img src="custom/themes/default/images/start_off.gif" alt="<?php echo $app_strings['LNK_LIST_START']; ?>" height="11" width="13" align="absmiddle" border="0">
						</button>
						
						<button id='scPrev' type="button" class="button" disabled="disabled" title="<?php echo $app_strings['LNK_LIST_PREVIOUS']; ?>">
						<img src="custom/themes/default/images/previous_off.gif" alt="<?php echo $app_strings['LNK_LIST_PREVIOUS']; ?>" height="11" width="8" align="absmiddle" border="0">
						</button>
						...
						<img src="custom/themes/default/images/next_off.gif" alt="<?php echo $app_strings['LNK_LIST_NEXT']; ?>" height="11" width="8" align="absmiddle" border="0">
						</button>
						
						<button id='scLast' type="button" class="button" disabled="disabled" title="<?php echo $app_strings['LNK_LIST_END']; ?>">
						<img src="custom/themes/default/images/end_off.gif" alt="<?php echo $app_strings['LNK_LIST_END']; ?>" height="11" width="13" align="absmiddle" border="0">
						</button>
				</div>
			</td>
		</tr>
...
foreach($ActRecords as $evt)
{
	...
	$start_time = $timedate->to_db_time($evt['time_start'],false);
...
?>
	...
	echo "
		...
		<button type='button' onclick='clearContacts(0);'>".$GLOBALS['app_strings']['LBL_CLEAR_BUTTON_LABEL']."</button>
