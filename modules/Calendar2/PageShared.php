...
global $currentModule, $current_language, $mod_strings;
				...
				if(!($i == $hour_start && $minute_start >= 0 + $t_step)){	
					echo "<div class='left_cell'>";
						if ($currentModule == "Home") {
							...
							echo timestamp_to_user_formated2($week_start_unix + $i * 3600,$GLOBALS['timedate']->get_time_format(false));
						} else {
							//echo timestamp_to_user_formated($week_start_unix + $i * 3600,$GLOBALS['timedate']->get_time_format());
							echo timestamp_to_user_formated2($week_start_unix + $i * 3600,$GLOBALS['timedate']->get_time_format());
						}
					echo "</div>";
				}
			...
			echo "<div class='day_col'>";
				...
				for($i = $hour_start; $i <= $hour_end; $i++){
					if($i == $hour_end && $minute_end == 0)
						break;
					if(!($i == $hour_start && $minute_start >= 0 + $t_step)){	
						echo "<div id='t_".$curr_time.$un_str."' class='".$un_class."' lang='".timestamp_to_user_formated2($curr_time,$GLOBALS['timedate']->get_time_format())."' datetime='".timestamp_to_user_formated2($curr_time)."'></div>";
					}
					$curr_time += $t_step*60;
					for($j = $t_step; $j < 60; $j += $t_step){
						if($i == $hour_end && $j >= $minute_end)
