 ...
 */
require_once 'modules/Calendar2/Zend/Validate/Abstract.php';

/**
 * @see Zend_Validate_Ip
 */
require_once 'modules/Calendar2/Zend/Validate/Ip.php';
...
class Zend_Validate_Hostname extends Zend_Validate_Abstract
{
    ...
    protected $_validIdns = array(
        ...
        'HU'  => array(1 => '/^[\x{002d}0-9a-záéíóöúü??]{1,63}$/iu'),
        'INFO'=> array(1 => '/^[\x{002d}0-9a-zäåæéöøü]{1,63}$/iu',
            2 => '/^[\x{002d}0-9a-záéíóöúü??]{1,63}$/iu',
            3 => '/^[\x{002d}0-9a-záæéíðóöúýþ]{1,63}$/iu',
            4 => '/^[\x{AC00}-\x{D7A3}]{1,17}$/iu',
            5 => '/^[\x{002d}0-9a-z??????????š?ž]{1,63}$/iu',
            6 => '/^[\x{002d}0-9a-z?????š??ž]{1,63}$/iu',
            7 => '/^[\x{002d}0-9a-zó????????]{1,63}$/iu',
            8 => '/^[\x{002d}0-9a-záéíñóúü]{1,63}$/iu'),
        'IO'  => array(1 => '/^[\x{002d}0-9a-zà-öø-ÿ??????????????????????????????????????œ??????š?????????????ž?]{1,63}$/iu'),
        ...
        'SH'  => array(1 => '/^[\x{002d}0-9a-zà-öø-ÿ??????????????????????????????????????œ??????š?????????????ž?]{1,63}$/iu'),
        'SJ'  => array(1 => '/^[\x{002d}0-9a-zàáä-éêñ-ôöøü????š?ž]{1,63}$/iu'),
        'TH'  => array(1 => '/^[\x{002d}0-9a-z\x{0E01}-\x{0E3A}\x{0E40}-\x{0E4D}\x{0E50}-\x{0E59}]{1,63}$/iu'),
        'TM'  => array(1 => '/^[\x{002d}0-9a-zà-öø-ÿ????????????????????????????????œ??????š????????????ž]{1,63}$/iu'),
        'TW'  => 'Zend/Validate/Hostname/Cn.php',
        'TR'  => array(1 => '/^[\x{002d}0-9a-z??ü?öç]{1,63}$/iu'),
        'VE'  => array(1 => '/^[\x{002d}0-9a-záéíóúüñ]{1,63}$/iu'),
        'VN'  => array(1 => '/^[ÀÁÂÃÈÉÊÌÍÒÓÔÕÙÚÝàáâãèéêìíòóôõùúý????????????\x{1EA0}-\x{1EF9}]{1,63}$/iu'),
                ...
                if (preg_match('/([^.]{2,10})$/i', end($domainParts), $matches) ||
                    ...
                    if ($this->_options['idn'] &&  isset($this->_validIdns[strtoupper($this->_tld)])) {
                        if (is_string($this->_validIdns[strtoupper($this->_tld)])) {
                            $regexChars += include('modules/Calendar2/'.$this->_validIdns[strtoupper($this->_tld)]);
                        } else {
