...
<?php
	...
	foreach($tabs as $tab) { 
?>
<input type="button" <?php if($args['view'] == $tab) {?>selected="selected" <?php } ?> value=" <?php echo $mod_strings["LBL_".$args['calendar']->get_view_name($tab)]; ?> " title="<?php echo $mod_strings["LBL_".$args['calendar']->get_view_name($tab)]; ?>" onclick="window.location.href='index.php?module=Calendar2&action=index&view=<?php echo $tab; ?><?php echo $args['calendar']->date_time->get_date_str(); ?>'">&nbsp;
<?php } ?>
</td>
</tr>
</table>
...
<?php
		...
		$newargs['calendar'] = new Calendar2('month', $cal_arr);
...
<?php
	...
	function template_echo_slice_activities(& $args) {
		...
		foreach($args['slice']->acts_arr[$current_user->id] as $act) {
			...
			if($act->sugar_bean->ACLAccess('DetailView') && file_exists('modules/' . $act->sugar_bean->module_dir . '/metadata/additionalDetails.php')) {
				...
				//Modified to avoid a module scanner in Sugar On-Demand
				//$ad_function = 'additionalDetails' . $act->sugar_bean->object_name;
				//$results = $ad_function($fields);
				$results = array();
				switch($act->sugar_bean->object_name) {
					case 'Meeting':
						$results = additionalDetailsMeeting($fields);
					case 'Call':
						$results = additionalDetailsCall($fields);
					case 'Task':
						$results = additionalDetailsTask($fields);
				}
				//End modification
			...
			$extra = "onmouseover=\"return overlib('" . 
					...
					. ", CLOSETEXT, '<img border=0  style=\'margin-left:2px; margin-right: 2px;\' src=".(is551()) ? SugarThemeRegistry::current()->getImageURL('close.gif') : "{$image_path}close.gif".">', "
			...
			if($act->sugar_bean->object_name == 'Call') { 
				echo '<td>' .(is551()) ? SugarThemeRegistry::current()->getImage('Calls','alt="'.$app_list_strings['call_status_dom'][$act->sugar_bean->status].': '.$act->sugar_bean->name.'"') : get_image($image_path.'Calls','alt="'.$app_list_strings['call_status_dom'][$act->sugar_bean->status].': '.$act->sugar_bean->name.'"') . '</td>
				...
				$out = '<td>' . (is551()) ? SugarThemeRegistry::current()->getImage('Meetings','alt="'.$app_list_strings['meeting_status_dom'][$act->sugar_bean->status].': '.$act->sugar_bean->name.'"') : get_image($image_path.'Meetings','alt="'.$app_list_strings['meeting_status_dom'][$act->sugar_bean->status].': '.$act->sugar_bean->name.'"'). '</td>
				...
				echo '<td>' . (is551()) ? SugarThemeRegistry::current()->getImage('Tasks','alt="'.$act->sugar_bean->status.': '.$act->sugar_bean->name.'"') : get_image($image_path.'Tasks','alt="'.$act->sugar_bean->status.': '.$act->sugar_bean->name.'"') . '</td>
	...
	function template_echo_slice_activities_shared(& $args) {
		...
		foreach($args['slice']->acts_arr[$shared_user->id] as $act) {
			...
			if($act->sugar_bean->object_name == 'Call') { 
				echo "<tr><td>";
				(is551()) ? SugarThemeRegistry::current()->getImage('Calls','alt=\"'.$app_list_strings['call_status_dom'][$act->sugar_bean->status].': '.$act->sugar_bean->name.'\"') : get_image($image_path.'Calls','alt=\"'.$app_list_strings['call_status_dom'][$act->sugar_bean->status].': '.$act->sugar_bean->name.'\"'); 
				echo "</td>";
				...
				echo "<td>".
					(is551()) ? SugarThemeRegistry::current()->getImage('Meetings','alt=\"'.$app_list_strings['meeting_status_dom'][$act->sugar_bean->status].': '.$act->sugar_bean->name.'\"') : get_image($image_path.'Meetings','alt=\"'.$app_list_strings['meeting_status_dom'][$act->sugar_bean->status].': '.$act->sugar_bean->name.'\"'); 
				echo "</td>";
				...
				echo "<td>".
					(is551()) ? SugarThemeRegistry::current()->getImage('Tasks','alt="'.$act->sugar_bean->status.': '.$act->sugar_bean->name.'"') : get_image($image_path.'Tasks','alt="'.$act->sugar_bean->status.': '.$act->sugar_bean->name.'"'); 
				echo "</td>";
	...
	function template_calendar(& $args) {
		...
		if(isset($_REQUEST['view']) && $_REQUEST['view'] == 'shared') {
			...
			echo "	</select></td>
				<td><a onclick=\"up('shared_ids');\">".(is551()) ? SugarThemeRegistry::current()->getImage('uparrow_big', 'border="0" style="margin-bottom: 1px;" alt="'.$app_strings['LBL_SORT'].'"') : get_image($image_path.'uparrow_big', 'border="0" style="margin-bottom: 1px;" alt="'.$app_strings['LBL_SORT'].'"')."</a><br>
				<a onclick=\"down('shared_ids');\">".(is551()) ? SugarThemeRegistry::current()->getImage('downarrow_big', 'border="0" style="margin-top: 1px;"  alt="'.$app_strings['LBL_SORT'].'"') : get_image($image_path.'downarrow_big', 'border="0" style="margin-top: 1px;"  alt="'.$app_strings['LBL_SORT'].'"')."</a></td>
			</tr>
			<tr>";
		...
		<a style="text-decoration: none;" 
			href="index.php?module=Calendar2&action=index&view=month&<?php echo $args['calendar']->date_time->get_date_str();?>">
<?php
...
function template_echo_date_info($view, $date_time) {
	...
	if($view == 'month' || $view == 'sharedmonthly') {
...
function template_get_next_calendar(& $args) {
	...
	global $current_language, $currentModule, $image_path, $mod_strings;
	//in case this is a dashlet
	$current_module_strings = return_module_language($current_language, 'Calendar2');

?>
<a href="index.php?action=index&module=<?php echo $currentModule; ?>&view=<?php echo $args['calendar']->view; ?>&<?php echo $args['calendar']->get_next_date_str(); ?>"><?php echo $current_module_strings["LBL_NEXT_".$args['calendar']->get_view_name($args['calendar']->view)]; ?>&nbsp;<?php echo (is551()) ? SugarThemeRegistry::current()->getImage('calendar_next','alt="'. $current_module_strings["LBL_NEXT_".$args['calendar']->get_view_name($args['calendar']->view)].'" align="absmiddle" border="0"') : get_image($image_path.'calendar_next','alt="'. $current_module_strings["LBL_NEXT_".$args['calendar']->get_view_name($args['calendar']->view)].'" align="absmiddle" border="0"'); ?></a>
<?php
...
?>
<a href="index.php?action=index&module=<?php echo $currentModule; ?>&view=<?php echo $args['calendar']->view; ?>&<?php echo $args['calendar']->get_previous_date_str(); ?>"><?php echo (is551()) ? SugarThemeRegistry::current()->getImage('calendar_previous','alt="'. $current_module_strings["LBL_PREVIOUS_".$args['calendar']->get_view_name($args['calendar']->view)].'" align="absmiddle" border="0"') : get_image($image_path.'calendar_previous','alt="'. $current_module_strings["LBL_PREVIOUS_".$args['calendar']->get_view_name($args['calendar']->view)].'" align="absmiddle" border="0"'); ?>&nbsp;&nbsp;<?php echo $current_module_strings["LBL_PREVIOUS_".$args['calendar']->get_view_name($args['calendar']->view)]; ?></a>
<?php
