...
class Calendar2ViewAjaxGetGRArr extends SugarView {
 	...
 	function display() {
		...
		$contacts_ids = explode(",",trim($_REQUEST['contacts'],','));
		$leads_ids = explode(",",trim($_REQUEST['leads'],','));
		
		$user_ids = array_unique($user_ids);
		$resource_ids = array_unique($resource_ids);
		$contacts_ids = array_unique($contacts_ids);
		$leads_ids = array_unique($leads_ids);
